...
func TestEnsureContentFolders(t *testing.T) {
	t.Run("creates content folders", func(t *testing.T) {
		tempDir := t.TempDir()

		cfg := ContentConfig{
			BasePath:    tempDir + "/dz_content",
			ThemesPath:  tempDir + "/dz_content/themes",
			PluginsPath: tempDir + "/dz_content/plugins",
			UploadsPath: tempDir + "/dz_content/uploads",
		}

		err := cfg.EnsureContentFolders()
		if err != nil {
			t.Fatalf("EnsureContentFolders() returned error: %v", err)
		}

		// Verify directories exist
		for _, path := range []string{cfg.BasePath, cfg.ThemesPath, cfg.PluginsPath, cfg.UploadsPath} {
	...
	t.Run("is idempotent", func(t *testing.T) {
		...
		err := cfg.EnsureContentFolders()
		if err != nil {
			t.Fatalf("EnsureContentFolders() first call returned error: %v", err)
		}

		err = cfg.EnsureContentFolders()
		if err != nil {
			t.Fatalf("EnsureContentFolders() second call returned error: %v", err)
		}
	})
}
